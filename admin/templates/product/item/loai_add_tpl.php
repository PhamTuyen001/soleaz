<?php
    ...
    function get_main_cat()
    {
        ...
        $str = '<select id="id_cat" name="data[id_cat]" class="form-control select2"><option value="0">Chọn danh mục</option>';
        foreach($row as $v)
        {
...
<section class="content">
    <form class="validation-form" novalidate method="post" action="<?=$linkSave?>" enctype="multipart/form-data">
        ...
        <div class="row">
            <div class="col-xl-8">
                <?php
                 if(!empty($config['product'][$type]['slug_item']) && $config['product'][$type]['slug_item']==true)
                {
                                ...
                                <div class="tab-content" id="custom-tabs-three-tabContent-lang">
                                    <?php foreach($config['website']['lang'] as $k => $v) { ?>
                                        <div class="tab-pane fade show <?=($k==$config['website']['lang-doc'])?'active':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
                                            ...
                                            <?php if(!empty($config['product'][$type]['mota_item']) && $config['product'][$type]['mota_item']==true) { ?>
                                                <div class="form-group">
                                            ...
                                            <?php } ?>
                                            <?php if(!empty($config['product'][$type]['noidung_item']) && $config['product'][$type]['noidung_item']==true) { ?>
                                                <div class="form-group">
                                                    <label for="noidung<?=$k?>">Nội dung (<?=$k?>):</label>
                                                    <textarea class="form-control for-seo <?=($config['product'][$type]['noidung_cke_item'])?'form-control-ckeditor':''?>" name="data[noidung<?=$k?>]" id="noidung<?=$k?>" rows="5" placeholder="Nội dung (<?=$k?>)"><?=(!empty($item['noidung'.$k])) ?htmlspecialchars_decode($item['noidung'.$k]):''?></textarea>
                                                </div>
                                            <?php } ?>
                                        </div>
                                    <?php } ?>
            ...
            <div class="col-xl-4">
                ...
                <?php if(!empty($config['product'][$type]['images_item']) && $config['product'][$type]['images_item']==true) { ?>
                    <div class="card card-primary card-outline text-sm">
                        <div class="card-header">
        ...
        <?php if(!empty($config['product'][$type]['seo_item']) && $config['product'][$type]['seo_item']==true) { ?>
            <div class="card card-primary card-outline text-sm">
                <div class="card-header">
