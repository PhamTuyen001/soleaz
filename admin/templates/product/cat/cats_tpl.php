            ...
            <table class="table table-hover">
                <thead>
                    <tr>
						...
						<?php if(!empty($config['product'][$type]['show_images_cat']) && $config['product'][$type]['show_images_cat']==true) { ?>
							<th class="align-middle">Hình</th>
						<?php } ?>
						<th class="align-middle" style="width:30%">Tiêu đề</th>
						<?php if(!empty($config['product'][$type]['check_cat'])){ foreach($config['product'][$type]['check_cat'] as $key => $value) { ?>
                    ...
                    <tbody>
                        <?php for($i=0;$i<count($items);$i++) {
                            ...
                            <tr>
                                ...
                                <?php if(!empty($config['product'][$type]['show_images_cat']) && $config['product'][$type]['show_images_cat']==true) { ?>
                                    <td class="align-middle">
                                    	<a href="<?=$linkEdit?><?=$linkID?>&id=<?=$items[$i]['id']?>" title="<?=$items[$i]['ten'.$config['website']['lang-doc']]?>"><img class="rounded img-preview" onerror="src='assets/images/noimage.png'" src="<?=THUMBS?>/<?=$config['product'][$type]['thumb_cat']?>/<?=UPLOAD_PRODUCT_L.$items[$i]['photo']?>" alt="<?=$items[$i]['ten'.$config['website']['lang-doc']]?>"></a>
                                    </td>
                                <?php } ?>
                                <td class="align-middle">
                                ...
                                <?php if(!empty($config['product'][$type]['check_cat'])){ foreach($config['product'][$type]['check_cat'] as $key => $value) { ?>
								  	<td class="align-middle text-center">
	                                	<div class="custom-control custom-checkbox my-checkbox">
	                                        <input type="checkbox" class="custom-control-input show-checkbox" id="show-checkbox-<?=$key?>-<?=$items[$i]['id']?>" data-table="product_cat" data-id="<?=$items[$i]['id']?>" data-loai="<?=$key?>" <?=($items[$i][$key]==true)?'checked':''?>>
