...
<section class="content">
    <form class="validation-form" novalidate method="post" action="<?=$linkSave?>" enctype="multipart/form-data">
        ...
        <div class="row">
            <div class="<?=$colLeft?>">
                <?php
                    if(!empty($config['product'][$type]['slug_brand']) && $config['product'][$type]['slug_brand']==true)
                    {
                                ...
                                <div class="tab-content" id="custom-tabs-three-tabContent-lang">
                                    <?php foreach($config['website']['lang'] as $k => $v) { ?>
                                        <div class="tab-pane fade show <?=($k==$config['website']['lang-doc'])?'active':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
                                            ...
                                            <?php if(!empty($config['product'][$type]['mota_brand']) && $config['product'][$type]['mota_brand']==true) { ?>
                                                <div class="form-group">
                                            ...
                                            <?php } ?>
                                            <?php if(!empty($config['product'][$type]['noidung_brand']) && $config['product'][$type]['noidung_brand']==true) { ?>
                                                <div class="form-group">
                                                    <label for="noidung<?=$k?>">Nội dung (<?=$k?>):</label>
                                                    <textarea class="form-control for-seo <?=($config['product'][$type]['noidung_cke_brand'])?'form-control-ckeditor':''?>" name="data[noidung<?=$k?>]" id="noidung<?=$k?>" rows="5" placeholder="Nội dung (<?=$k?>)"><?=(!empty($item['noidung'.$k])) ? htmlspecialchars_decode($item['noidung'.$k]):''?></textarea>
                                                </div>
                                            <?php } ?>
                                        </div>
                                    <?php } ?>
            ...
            <div class="<?=$colRight?>">
                <?php if(!empty($config['product'][$type]['images_brand']) && $config['product'][$type]['images_brand']==true) { ?>
                    <div class="card card-primary card-outline text-sm">
                        <div class="card-header">
        ...
        <?php if(!empty($config['product'][$type]['seo_brand']) && $config['product'][$type]['seo_brand']==true) { ?>
            <div class="card card-primary card-outline text-sm">
                <div class="card-header">
