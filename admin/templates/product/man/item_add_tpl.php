<?php
	...
	function get_main_list()
    {
        ...
        $row = $d->rawQuery("select tenen, id from #_product_list where type = ? order by stt,id desc",array($type));
	...
	function get_main_cat()
	{
        ...
        $id_list = isset($_REQUEST['id_list']) ? $_REQUEST['id_list']:'';
        $row = $d->rawQuery("select tenen, id from #_product_cat where id_list = ? and type = ? order by stt,id desc",array($id_list,$type));
	...
	function get_main_item()
	{
        ...
        $id_list = isset($_REQUEST['id_list']) ? $_REQUEST['id_list']:'';
        $id_cat = isset($_REQUEST['id_cat']) ? $_REQUEST['id_cat']:'';
        $row = $d->rawQuery("select tenen, id from #_product_item where id_list = ? and id_cat = ? and type = ? order by stt,id desc",array($id_list,$id_cat,$type));
	...
	function get_main_sub()
	{
        ...
        $id_cat = isset($_REQUEST['id_cat']) ? $_REQUEST['id_cat']:'';
        $id_item = isset($_REQUEST['id_item']) ? $_REQUEST['id_item']:'';
        $row = $d->rawQuery("select tenen, id from #_product_sub where id_list = ? and id_cat = ? and id_item = ? and type = ? order by stt,id desc",array($id_list,$id_cat,$id_item,$type));
	...
	function get_main_brand()
    {
        ...
        $row = $d->rawQuery("select tenen, id from #_product_brand where type = ? order by stt,id desc",array($type));
	...
	function get_tags($id="")
	{
		...
		$row_tags = $d->rawQuery("select tenen, id from #_tags where type = ? order by stt,id desc",array($type));
	...
	function get_mau($id="")
	{
		...
		$row_mau = $d->rawQuery("select tenen, id from #_product_mau where type = ? order by stt,id desc",array($type));
		...
		for($i=0;$i<count($row_mau);$i++)
		{
			...
			$str .= '<option value="'.$row_mau[$i]["id"].'" '.(!empty($selected) ? $selected:'').' /> '.$row_mau[$i]["tenen"].'</option>';
		}
		$str .= '</select>';
	...
	function get_size($id="")
	{
		...
		$row_size = $d->rawQuery("select tenen, id from #_product_size where type = ? order by stt,id desc",array($type));
		...
		for($i=0;$i<count($row_size);$i++)
		{
			...
			$str .= '<option value="'.$row_size[$i]["id"].'" '.(!empty($selected) ? $selected:'').' /> '.$row_size[$i]["tenen"].'</option>';
		}
		$str .= '</select>';
    ...
    <form class="validation-form" novalidate method="post" action="<?=$linkSave?>" enctype="multipart/form-data">
        ...
        <div class="row">
        	...
        	<div class="<?=$colRight?>">
		                ...
		                <div class="row">
							...
							<?php if(!empty($product_type['gia']) && $product_type['gia']==true) { ?>
						    	<div class="form-group col-md-6">
			                        ...
			                        <div class="input-group">
			                        	<input type="text" class="form-control gia_ban" name="data[gia]" id="gia" placeholder="Giá bán" value="<?=(!empty($item['gia'])) ? $item['gia']:''?>">
						    ...
						    <?php if(!empty($product_type['giamoi']) && $product_type['giamoi']==true) { ?>
						    	<div class="form-group col-md-6">
			                        ...
			                        <div class="input-group">
			                        	<input type="text" class="form-control gia_moi" name="data[giamoi]" id="giamoi" placeholder="Giá mới" value="<?=(!empty($item['giamoi'])) ? $item['giamoi']:''?>">
			                        	<div class="input-group-append">
			                        		<div class="input-group-text"><strong>USD</strong></div>
			                        	</div>
			                        </div>
		        ...
		        <?php if(!empty($product_type['images2']) && $product_type['images2']==true) { ?>
			        <div class="card card-primary card-outline text-sm">
                    <div class="card-header">
                        <h3 class="card-title">Hình ảnh 2 <?=$product_type['title_main']?></h3>
                        <div class="card-tools">
                            <button type="button" class="btn btn-tool" data-card-widget="collapse"><i class="fas fa-minus"></i></button>
                        </div>
                    </div>
                    <div class="card-body">
                        <div class="form-group">
                            <label class="change-photo" for="file2">
                                <p>Upload hình ảnh: <strong class="mt-2 mb-2 text-sm text-danger"><?php echo "Width: ".$product_type['width']." px - Height: ".$product_type['height']." px (".$product_type['img_type'].")"; ?></strong></p>
                                <div class="rounded">
                                    <img class="rounded img-upload" src="<?=UPLOAD_PRODUCT.$item['photo2']?>" onerror="src='assets/images/noimage.png'" alt="Alt Photo"/>
