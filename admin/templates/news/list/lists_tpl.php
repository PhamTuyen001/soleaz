            ...
            <table class="table table-hover">
                <thead>
                    <tr>
                        ...
                        <?php } ?>
						<?php if(isset($config['news'][$type]['gallery_list'])) { foreach($config['news'][$type]['check_list'] as $key => $value) { ?>
                    ...
                    <tbody>
                        <?php for($i=0;$i<count($items);$i++) { ?>
                            <tr>
                                ...
                                <?php if(isset($config['news'][$type]['show_images_list']) && $config['news'][$type]['show_images_list']==true) { ?>
                                    <td class="align-middle">
                                    	<a href="<?=$linkEdit?>&id=<?=$items[$i]['id']?>" title="<?=$items[$i]['ten'.$config['website']['lang-doc']]?>"><img class="rounded img-preview" onerror="src='assets/images/noimage.png'" src="<?=THUMBS?>/<?=$config['news'][$type]['thumb_list']?>/<?=UPLOAD_NEWS_L.$items[$i]['photo']?>" alt="<?=$items[$i]['ten'.$config['website']['lang-doc']]?>"></a>
                                    </td>
                                <?php } ?>
                                <td class="align-middle">
                                ...
                                <?php if(isset($config['news'][$type]['gallery_list'])) { foreach($config['news'][$type]['check_list'] as $key => $value) { ?>
								  	<td class="align-middle text-center">
	                                	<div class="custom-control custom-checkbox my-checkbox">
	                                        <input type="checkbox" class="custom-control-input show-checkbox" id="show-checkbox-<?=$key?>-<?=$items[$i]['id']?>" data-table="news_list" data-id="<?=$items[$i]['id']?>" data-loai="<?=$key?>" <?=($items[$i][$key]==true)?'checked':''?>>
