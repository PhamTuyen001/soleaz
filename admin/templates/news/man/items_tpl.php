<?php 
	...
	function get_main_sub()
	{
        ...
        $id_cat = (isset($_REQUEST['id_cat'])) ? htmlspecialchars($_REQUEST['id_cat']):'';
        $id_item = (isset($_REQUEST['id_item'])) ? htmlspecialchars($_REQUEST['id_item']):'';
        $row = $d->rawQuery("select tenen, id from #_news_sub where id_list = ? and id_cat = ? and id_item = ? and type = ? order by stt,id desc",array($id_list,$id_cat,$id_item,$type));
                    ...
                    <tbody>
                        <?php for($i=0;$i<count($items);$i++) {
                            ...
                            <tr>
                                ...
                                <?php if(isset($config['news'][$type]['show_images']) && $config['news'][$type]['show_images']) { ?>
                                    <td class="align-middle">
                                    	<a href="<?=$linkEdit?><?=$linkID?>&id=<?=$items[$i]['id']?>" title="<?=$items[$i]['ten'.$config['website']['lang-doc']]?>"><img class="rounded img-preview" onerror="src='assets/images/noimage.png'" src="<?=THUMBS?>/<?=$config['news'][$type]['thumb']?>/<?=UPLOAD_NEWS_L.$items[$i]['photo']?>" alt="<?=$items[$i]['ten'.$config['website']['lang-doc']]?>"></a>
                                    </td>
                                <?php } ?>
                                <td class="align-middle">
