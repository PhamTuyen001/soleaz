                    ...
                    <tbody>
                        <?php for($i=0;$i<count($items);$i++) {
                            ...
                            <tr>
                                ...
                                <?php if(isset($config['news'][$type]['show_images_item']) && $config['news'][$type]['show_images_item']==true) { ?>
                                    <td class="align-middle">
                                    	<a href="<?=$linkEdit?><?=$linkID?>&id=<?=$items[$i]['id']?>" title="<?=$items[$i]['ten'.$config['website']['lang-doc']]?>"><img class="rounded img-preview" onerror="src='assets/images/noimage.png'" src="<?=THUMBS?>/<?=$config['news'][$type]['thumb_item']?>/<?=UPLOAD_NEWS_L.$items[$i]['photo']?>" alt="<?=$items[$i]['ten'.$config['website']['lang-doc']]?>"></a>
                                    </td>
                                <?php } ?>
                                <td class="align-middle">
                                ...
                                <?php if(isset($config['news'][$type]['check_item'])){ foreach($config['news'][$type]['check_item'] as $key => $value) { ?>
								  	<td class="align-middle text-center">
	                                	<div class="custom-control custom-checkbox my-checkbox">
	                                        <input type="checkbox" class="custom-control-input show-checkbox" id="show-checkbox-<?=$key?>-<?=$items[$i]['id']?>" data-table="news_item" data-id="<?=$items[$i]['id']?>" data-loai="<?=$key?>" <?=($items[$i][$key]==true)?'checked':''?>>
